...
class LSTMCell:
    def __init__(self, input_size, hidden_size):
        ...
        self.W_f = np.random.randn(input_size + hidden_size, hidden_size) * 0.01  # Forget gate (Unutma kapısı)
        self.b_f = np.zeros((1, hidden_size))
        
        # Weights for forget gate (Unutma kapısı için ağırlıklar)
        self.W_i = np.random.randn(input_size + hidden_size, hidden_size) * 0.01  # Input gate (Giriş kapısı)
        self.b_i = np.zeros((1, hidden_size))

        # Weights for cell gate (Hücre kapısı için ağırlıklar)
        self.W_c = np.random.randn(input_size + hidden_size, hidden_size) * 0.01  # Cell gate (Hücre kapısı)
        self.b_c = np.zeros((1, hidden_size))

        # Weights for output gate (Çıkış kapısı için ağırlıklar)
        self.W_o = np.random.randn(input_size + hidden_size, hidden_size) * 0.01  # Output gate (Çıkış kapısı)
        self.b_o = np.zeros((1, hidden_size))

        # Initial hidden state and cell state (Başlangıç gizli durumu ve hücre durumu)
        self.h = np.zeros((1, hidden_size))  # Hidden state (Gizli durum)
        self.c = np.zeros((1, hidden_size))  # Cell state (Hücre durumu)
    ...
    def backward(self, x, y_true, y_pred, learning_rate=0.01):
        ...
        """
        pass  # Placeholder for backpropagation logic (Geri yayılım mantığı için yer tutucu)
...
if __name__ == "__main__":
    # Example input and output sequences (Örnek bir girdi ve çıktı dizisi oluşturalım)
    X = np.random.randn(10, 3)  # 10 time steps, 3 features per step (10 zaman adımı, her adımda 3 özellik)
    y = np.random.randn(10, 1)  # 10 time steps, 1 output per step (10 zaman adımı için 1 çıktı)

    # Initialize the LSTM model (LSTM modelini başlat)
    input_size = 3  # Number of features per time step (Her zaman adımındaki özellik sayısı)
    hidden_size = 5  # Number of hidden units in the LSTM (LSTM'deki gizli nöron sayısı)
    output_size = 1  # Number of outputs (Çıktı sayısı)

    lstm_model = LSTMModel(input_size, hidden_size, output_size)

    # Train the LSTM model (LSTM modelini eğit)
    lstm_model.train(X, y, epochs=1000, learning_rate=0.01)

    # Make a prediction after training (Eğitim sonrası bir tahmin yapalım)
