...
class GradientDescentLogisticRegression:
    def __init__(self, learning_rate=0.01, epochs=1000, batch_size=None):
        """
        Initializes the Logistic Regression model with gradient descent.
        Gradient descent ile Logistic Regression modelini başlatır.
        ...
        :param batch_size: Size of mini-batches for mini-batch gradient descent (Mini-batch gradient descent için batch boyutu)
                          If None, stochastic gradient descent is used (Eğer None ise stochastic gradient descent kullanılır)
        """
    ...
    def fit(self, X, y):
        ...
        Gradient descent kullanarak logistic regression modelini eğitir.
        ...
        for epoch in range(self.epochs):
            if self.batch_size is None:  # Stochastic Gradient Descent (SGD)
                for i in range(n_samples):
                    ...
                    dw, db = self.compute_gradients(xi, yi, y_predicted)
    ...
    def cross_entropy_loss(self, y_true, y_pred):
        ...
        y_pred = np.clip(y_pred, 1e-9, 1 - 1e-9)  # Avoid log(0) error (log(0) hatasını önlemek için)
...
if __name__ == "__main__":
    ...
    # Initialize the model with Stochastic Gradient Descent (SGD ile modeli başlat)
    classifier_sgd = GradientDescentLogisticRegression(learning_rate=0.01, epochs=1000, batch_size=None)

    # Train the model (Modeli eğit)
    classifier_sgd.fit(X, y)

    # Predict class labels (Sınıf etiketlerini tahmin et)
    predictions_sgd = classifier_sgd.predict(X)
    print(f"Predictions with Stochastic Gradient Descent: {predictions_sgd}")

    # Initialize the model with Mini-Batch Gradient Descent (Mini-batch gradient descent ile modeli başlat)
    classifier_mbgd = GradientDescentLogisticRegression(learning_rate=0.01, epochs=1000, batch_size=2)

    # Train the model (Modeli eğit)
    classifier_mbgd.fit(X, y)

    # Predict class labels (Sınıf etiketlerini tahmin et)
    predictions_mbgd = classifier_mbgd.predict(X)
    print(f"Predictions with Mini-Batch Gradient Descent: {predictions_mbgd}")
